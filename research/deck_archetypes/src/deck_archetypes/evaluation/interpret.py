...
archetype themselves ("Firepower, Combat Training, Boot Camp, ..."). Writes a
readable text report and returns a compact summary.
"""
...
def report(corpus, labels, cfg, out_dir):
    ...
    for c in clusters:
        ...
        summary.append({"cluster": int(c), "size": int(len(rows)), "top_cards": top_cards[:8]})
