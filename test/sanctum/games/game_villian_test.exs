defmodule Sanctum.Games.GameVillianTest do
  ...
  describe "change_health" do
    test "increases health when amount is positive" do
      ...
      assert {:ok, updated_villain} = Games.change_villain_health(game_villain, %{amount: 2}, actor: user)
      assert updated_villain.health == 14
    end

    test "decreases health when amount is negative" do
      {user, _game, game_villain} = create_test_game_villain()
      assert {:ok, updated_villain} = Games.change_villain_health(game_villain, %{amount: -5}, actor: user)
      assert updated_villain.health == 7
    end

    test "allows health to go to zero" do
      {user, _game, game_villain} = create_test_game_villain()
      assert {:ok, updated_villain} = Games.change_villain_health(game_villain, %{amount: -12}, actor: user)
      assert updated_villain.health == 0
    end

    test "prevents health from going below zero" do
      {user, _game, game_villain} = create_test_game_villain()
      assert {:ok, updated_villain} = Games.change_villain_health(game_villain, %{amount: -20}, actor: user)
      assert updated_villain.health == 0
    end

    test "caps health at max_health when increasing" do
      {user, _game, game_villain} = create_test_game_villain()
      assert {:ok, updated_villain} = Games.change_villain_health(game_villain, %{amount: 10}, actor: user)
    ...
    test "handles zero amount" do
      ...
      assert {:ok, updated_villain} = Games.change_villain_health(game_villain, %{amount: 0}, actor: user)
      assert updated_villain.health == original_health
    end

    test "requires amount argument" do
      {user, _game, game_villain} = create_test_game_villain()
      assert {:error, %Ash.Error.Invalid{}} = Games.change_villain_health(game_villain, %{}, actor: user)
    end

    test "requires integer amount" do
      {user, _game, game_villain} = create_test_game_villain()
      assert {:error, %Ash.Error.Invalid{}} = Games.change_villain_health(game_villain, %{amount: "not_a_number"}, actor: user)
    end
  end
end